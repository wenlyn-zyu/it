...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{../rd-semantic-repro/figures/fig2_five_regions.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{../rd-semantic-repro/figures/fig3_sordf_3d_contour.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{../rd-semantic-repro/figures/fig4_left_gx.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{../rd-semantic-repro/figures/fig4_right_rd.png}}
...
Besides the quantitative trade-off curves, we also examine the visual quality of the reconstructed images. Figures~\ref{fig:recon8} and~\ref{fig:recon16} compare the original MNIST digits (top row) with the corresponding RDPC reconstructions (bottom row) under two representative bottleneck dimensions.

For both latent dimensions, the reconstructed digits remain visually recognizable, indicating that the semantic information required for digit classification is largely preserved despite compression. Although some stroke details become smoother after reconstruction, the pretrained classifier still achieves high recognition accuracy, consistent with the quantitative results in Table~\ref{tab:sweep}.

Increasing the latent dimension from $k=8$ to $k=16$ leads to only modest improvements in visual appearance, while the classification accuracy changes very little. This observation qualitatively supports the SORDF intuition that additional coding resources are primarily used to improve appearance fidelity once the semantic information has already been captured.
\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{../figures/fig9.png}
\caption{Qualitative reconstruction results of the proposed RDPC model with latent dimension $k=8$. The first row shows the original MNIST digits, while the second row shows the corresponding reconstructed images. Despite the compact bottleneck, the semantic identity of each digit is well preserved.}
\label{fig:recon8}
\end{figure}
...
\caption{Qualitative reconstruction results with latent dimension $k=16$. Compared with $k=8$, the visual quality improves only slightly, suggesting that increasing the bottleneck mainly enhances appearance fidelity while providing limited additional semantic benefit.}
\label{fig:recon16}
\end{figure}
...
\begin{enumerate}
    ...
    \item \textbf{Qualitative semantic preservation}: Figures~\ref{fig:recon8} and~\ref{fig:recon16} show that the reconstructed digits remain visually recognizable across different bottleneck dimensions. Increasing the latent dimension from $k=8$ to $k=16$ leads to only limited perceptual improvement, while the semantic identity of each digit is already preserved at the smaller bottleneck. This qualitative evidence complements the quantitative accuracy results and supports the semantic communication principle that semantic information can be retained using substantially fewer coding resources than appearance information.
\end{enumerate}
